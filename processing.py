...
class Player:
    ...
    def internal_vision(self, screen, map, angle):
        for rays in range(-21, 22):
            # print('Start loop')
            distance = 0
            for sight in range(1, 45):
                line_x = self.pX + 7 + math.cos(math.radians(angle + (rays * 1.25))) * (sight * 5)
                line_y = self.pY + 7 + math.sin(math.radians(angle + (rays * 1.25))) * (sight * 5)
                ...
                output.draw_vision(screen, (distance * 4), rays)
                # print(distance)
# Identify tiles
